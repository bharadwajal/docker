...
then
  ...
  curl -X POST http://54.172.195.237:8080/job/docker-test1/build \
...
then
    ...
    curl -X POST http://54.172.195.237:8080/job/docker-test2/build \
  --user bj:b6475f8b607c0a88dfb82dca1fe02fe2 \
  --data-urlencode json='{"parameter": [{"name":"Name", "value":"Woohoo!"}]}'
...
then
    ...
    curl -X POST http://54.172.195.237:8080/job/docker-test3/build \
  --user bj:b6475f8b607c0a88dfb82dca1fe02fe2 \
  --data-urlencode json='{"parameter": [{"name":"Name", "value":"Woohoo!"}]}'
